...
# ConstDef -> Ident(10) { '[' ConstExp ']' } '=' ConstInitVal
def ConstDef(p, n, pr, v):
    if getSym(p, n, v) == 10:
        pr.append(10)
        while getSym(p, n, v)==322:#[
            pr.append(322)
...
# VarDef -> Ident(10) { '[' ConstExp ']' } VarDefA
def VarDef(p, n, pr, v):
    if getSym(p, n, v) == 10:
        pr.append(10)
        a=getSym(p,n,v)
...
# Stmt -> LVal '=' Exp ';' | [Exp] ';' | 'return' Exp ';' | Block | 'if' '(' Cond ')' Stmt [ 'else' Stmt | 'while' '(' Cond ')' Stmt ]| 'break' ';' | 'continue' ';'
# 12以分号前是否读到等号区分
# 3首字为‘return'
# 4首字为’{‘
# 5首字为'if'
